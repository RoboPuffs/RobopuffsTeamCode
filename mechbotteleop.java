...
public class MechBotAutoDriveByEncoder extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        robot.lbm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        robot.lfm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        robot.rbm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        robot.rfm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        telemetry.addData("Path0",  "Starting at %7d :%7d",
                robot.lbm.getCurrentPosition(),
                robot.lfm.getCurrentPosition(),
                robot.rbm.getCurrentPosition(),
                robot.rfm.getCurrentPosition());
        telemetry.update();
...
//        encoderDrive(DRIVE_SPEED, -24, -24, 4.0);  // S3: Reverse 24 Inches with 4 Sec timeout
    ...
    public void encoderMove(double speed, double leftBackMotorInches, double leftFrontMotorInches, double rightBackMotorInches, double rightFrontMotorInches, double timeoutS) {
        ...
        if (opModeIsActive()) {
            ...
            newLeftBackTarget = robot.lbm.getCurrentPosition() + (int)(leftBackMotorInches * COUNTS_PER_INCH);
            newLeftFrontTarget = robot.lfm.getCurrentPosition() + (int)(leftFrontMotorInches * COUNTS_PER_INCH);
            newRightBackTarget = robot.rbm.getCurrentPosition() + (int)(rightBackMotorInches * COUNTS_PER_INCH);
            newRightFrontTarget = robot.rfm.getCurrentPosition() + (int)(rightFrontMotorInches * COUNTS_PER_INCH);

            robot.lbm.setTargetPosition(newLeftBackTarget);
            robot.lfm.setTargetPosition(newLeftFrontTarget);
            robot.rbm.setTargetPosition(newRightBackTarget);
            robot.rfm.setTargetPosition(newRightFrontTarget);
            ...
            while (opModeIsActive() && (runtime.seconds() < timeoutS) && (robot.lbm.isBusy()
                    && robot.lfm.isBusy() && robot.rbm.isBusy() && robot.rfm.isBusy())) {
                ...
                telemetry.addData("Path2",  "Running at %7d :%7d",
                        robot.lbm.getCurrentPosition(), robot.lfm.getCurrentPosition(),
                        robot.rbm.getCurrentPosition(), robot.rfm.getCurrentPosition());
                telemetry.update();
            }
